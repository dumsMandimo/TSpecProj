...
function validateNoHeaderRows(qualifications) {
  const badRows = qualifications.filter(
    (q) => q.saqa_id === "Qual / Prog ID" || q.title === "Qualification Title / Learning Programme Title"
  );

  assert(badRows.length === 0, "Header row was accidentally saved as data");
}

function validateDuplicates(qualifications) {
...
function main() {
  ...
  validateDuplicates(qualifications);
