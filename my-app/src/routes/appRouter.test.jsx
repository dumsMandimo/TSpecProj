...
jest.mock('../pages/admin/Opportunities', () => () => <div>Opportunities Page</div>);
jest.mock('../pages/admin/Users', () => () => <div>Users Page</div>);
...
// Helper — renders AppRouter but overrides the browser history
// with a MemoryRouter pointing to a specific path
function renderAtPath(path) {
  // AppRouter has its own BrowserRouter so we render it directly
  // and use window.history to navigate
...
describe('AppRouter', () => {
  ...
  test('admin layout renders alongside admin dashboard', () => {
    renderAtPath('/dashboard/admin');
    expect(screen.getByText('Admin Layout')).toBeInTheDocument();
    expect(screen.getByText('Admin Dashboard')).toBeInTheDocument();
  });

  test('admin layout renders alongside opportunities page', () => {
    renderAtPath('/dashboard/admin/opportunities');
    expect(screen.getByText('Admin Layout')).toBeInTheDocument();
    expect(screen.getByText('Opportunities Page')).toBeInTheDocument();
  });

  test('admin layout renders alongside users page', () => {
    renderAtPath('/dashboard/admin/users');
    expect(screen.getByText('Admin Layout')).toBeInTheDocument();
    expect(screen.getByText('Users Page')).toBeInTheDocument();
  });
  ...
  test('applicant dashboard does not render at provider route', () => {
    renderAtPath('/dashboard/provider');
    expect(screen.queryByText('Applicant Dashboard')).not.toBeInTheDocument();
  });
